...
class PlantAnthropicClient:
    ...
    async def get_plant_details(self, plant_name: str) -> Dict:
        ...
        logger.info(
            f"Request {request_id} - calling Anthropic API for plant: {plant_name}"
        )
        ...
        try:
            ...
            if e.status_code in [401, 403]:
                logger.error(
                    f"Request {request_id} - Anthropic authentication error: {str(e)}"
                )
                raise PlantServiceException(
    ...
    def _validate_plant_details(self, details: Dict) -> None:
        ...
        if missing_fields:
            logger.warning(
                f"Missing fields in plant details: {', '.join(missing_fields)}"
            )
            raise PlantServiceException(
